...
/**
 * Returns the installed Angular-Cli version
 */
export function getAngularCliVersion(): string | undefined {
  try {
    // --no-install ensures that we are checking for an installed version
    // (either locally or globally)
    const npx = os.platform() === 'win32' ? 'npx.cmd' : 'npx';
    const angularCli = spawnSync(npx, ['--no-install', 'ng', '--version']);
    if (angularCli.status !== 0 || angularCli.error) {
      return undefined;
    }
    return angularCli.stdout.toString().trim();
  } catch (err) {
    return undefined;
  }
}
